...
projectStream :: (MonadSafe m, PersistBackend m, PersistEntity v, EntityConstr v c, Projection' p (Conn m) (RestrictionHolder v c) a, HasSelectOptions opts (Conn m) (RestrictionHolder v c))
              => p
              -> opts
              -> Producer' a m ()
projectStream p opts = fromStream $ Core.projectStream p opts

queryRawStream :: (MonadSafe m, PersistBackend m)
               => Bool           -- ^ keep in cache
               -> String         -- ^ query
