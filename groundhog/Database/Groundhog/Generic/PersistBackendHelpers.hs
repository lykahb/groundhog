...
selectStream conf@RenderConfig {..} queryFunc preColumns noLimit options = doSelectQuery
  where
...
selectAllStream RenderConfig {..} queryFunc = start
  where
    start = joinStreams $ zipWith selectConstr [0 ..] $ constructors e
    selectConstr cNum constr = queryFunc query [] >>= mapStream (mkEntity cNum)
      where
...
projectStream conf@RenderConfig {..} queryFunc preColumns noLimit p options = doSelectQuery
  where
    SelectOptions cond limit offset ords dist _ = getSelectOptions options
    e = entityDef proxy (undefined :: v)
    proxy = undefined :: proxy conn
...
replace RenderConfig {..} queryFunc execFunc insertIntoConstructorTable k v = do
  ...
  let e = entityDef proxy v
      ...
      RenderS upds updsVals = commasJoin $ zipWith f fields $ tail vals
        where
...
delete ::
  ...
  Cond conn r ->
  Action conn ()
delete conf@RenderConfig {..} execFunc cond = execFunc query (maybe [] (($ []) . getValues) cond')
  where
    e = entityDef proxy (undefined :: v)
    proxy = undefined :: proxy conn
    constr = constructors e !! entityConstrNum (undefined :: proxy v) (undefined :: c a)
    ...
    query =
      if isSimple (constructors e)
        then "DELETE FROM " <> tableName esc e constr <> whereClause
          ...
          "DELETE FROM " <> mainTableName esc e <> " WHERE id IN(SELECT " <> fromJust (constrId esc constr) <> " FROM " <> tableName esc e constr <> whereClause <> ")"
...
deleteBy ::
  ...
  Key v BackendSpecific ->
  Action conn ()
deleteBy RenderConfig {..} execFunc k = execFunc query [toPrimitivePersistValue k]
  where
...
deleteAll ::
  ...
  v ->
  Action conn ()
deleteAll RenderConfig {..} execFunc (_ :: v) = execFunc query []
  where
    e = entityDef proxy (undefined :: v)
    proxy = undefined :: proxy conn
