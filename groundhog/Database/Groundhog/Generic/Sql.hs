...
prerenderExpr conf (Expr e) = Expr $ ExprRaw typ $ Snippet $ \_ _ -> prerendered
  where
...
mkExpr snippet = Expr $ ExprRaw typ snippet
  where
    proxy = undefined :: proxy db
    typ = dbType proxy (undefined :: a)
...
renderCondPriority ::
  ...
  Cond db r ->
  Maybe (RenderS db r)
renderCondPriority conf@RenderConfig {..} priority cond = go cond priority
  where
    ...
    go (Compare compOp f1 f2) p =
      ...
      where
        ...
        renderZip3 opP ops expr1 expr2 = zipWith3 (\op e1 e2 -> op e1 e2) ops expr1' expr2'
          where
            expr1' = renderExprExtended conf opP expr1
            expr2' = renderExprExtended conf opP expr2
    ...
    perhaps p pOuter op a b = result
      where
...
flattenP escape prefix (fname, typ) acc = go typ
  where
    go typ' = case typ' of
...
tableName esc e c = qualifySchema esc e tName
  where
    tName =
      esc $
...
mainTableName esc e = qualifySchema esc e tName
  where
